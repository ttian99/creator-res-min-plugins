...
function resMin(buildPath, cb) {
  ...
  var resMinPath = path.join(Editor.projectPath, buildPath, 'res/raw-assets/**/*.*');
  var opt = {
    cache: false,
    pngQuality: '50',
    jpgQuality: '70'
...
function start() {
  Editor.log('== start ==');
  Editor.log('Editor.projectPath = ' + Editor.projectPath);
  resMin(QQPLAY_PATH, function () {
    Editor.info('== over ==');
  });
}
